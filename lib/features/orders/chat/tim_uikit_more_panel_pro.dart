...
class MorePanelPro extends StatefulWidget {
  /// 会话ID
  ...
  @override
  State<StatefulWidget> createState() => _MorePanelProState();
}

class _MorePanelProState extends TIMUIKitState<MorePanelPro> {
    ...
    return [
      ...
      if (!PlatformUtils().isWeb)
        MorePanelItem(
          ...
          title: TIM_t("照片"),
          onTap: (c) {
            _onFeatureTap("photo", c, model, theme);
          },
          icon: Container(
            height: 64,
            width: 64,
      ...
      if (PlatformUtils().isWeb)
        MorePanelItem(
          ...
          title: TIM_t("图片"),
          onTap: (c) {
            _onFeatureTap("image", c, model, theme);
          },
          icon: Container(
            height: 64,
            width: 64,
      ...
      if (PlatformUtils().isWeb)
        MorePanelItem(
          ...
          title: TIM_t("视频"),
          onTap: (c) {
            _onFeatureTap("video", c, model, theme);
          },
          icon: Container(
            height: 64,
            width: 64,
            ...
            child: Icon(Icons.video_file, color: Color(0xFF000002), size: 26),
          ),
        ),
      MorePanelItem(
        id: "file",
      ...
      if (isInstallCallkit && !kIsWeb)
        MorePanelItem(
          ...
          title: TIM_t("视频通话"),
          onTap: (c) {
            _onFeatureTap("videoCall", c, model, theme);
          },
          icon: Container(
            height: 64,
            width: 64,
      ...
      if (isInstallCallkit && !kIsWeb)
        MorePanelItem(
          ...
          title: TIM_t("语音通话"),
          onTap: (c) {
            _onFeatureTap("voiceCall", c, model, theme);
          },
          icon: Container(
            height: 64,
            width: 64,
  ...
  Future<void> _sendImageFromCamera(
    TUIChatSeparateViewModel model,
    TUITheme theme,
  ) async {
    try {
  ...
  Future<void> _sendImageFileOnWeb(TUIChatSeparateViewModel model) async {
    try {
  ...
  Future<void> _sendVideoFileOnWeb(TUIChatSeparateViewModel model) async {
    try {
  ...
  Future<void> _sendFile(TUIChatSeparateViewModel model, TUITheme theme) async {
    try {
      final convID = widget.conversationID;
      final convType = widget.conversationType;
      FilePickerResult? result = await FilePicker.platform.pickFiles();
      if (result != null && result.files.isNotEmpty) {
        ...
        outputLogger.i(
          TIM_t_para("选择成功{{option2}}", "选择成功$option2")(option2: option2),
        );
  ...
  Future<void> _goToVideoUI(String type) async {
    if (!PlatformUtils().isWeb) {
    ...
    if (isGroup) {
      List<V2TimGroupMemberFullInfo>? selectedMember = await Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) =>
              SelectCallInviter(groupID: widget.conversationID),
        ),
      );
