...
import 'narrow_pro.dart';

enum MuteStatus { none, me, all }

class TIMUIKitInputTextFieldPro extends StatefulWidget {
  /// conversation id
  ...
  const TIMUIKitInputTextFieldPro({
    super.key,
  ...
  @override
  State<StatefulWidget> createState() => _InputTextFieldProState();
}

class _InputTextFieldProState extends TIMUIKitState<TIMUIKitInputTextFieldPro> {
  ...
  generateStickerList() {
    ...
    if (stickerConfig.useTencentCloudChatStickerPackage) {
      final tccEmojiSet = TUIKitStickerConstData.emojiList.firstWhere(
        (element) => element.name == "tcc1",
      );
      stickerPackageList.add(
        CustomStickerPackage(
          ...
          stickerList: tccEmojiSet.list
              ...
              .toList(),
          menuItem: CustomSticker(index: 0, name: tccEmojiSet.icon),
        ),
      );
    }
    if (stickerConfig.useQQStickerPackage) {
      final qqEmojiSet = TUIKitStickerConstData.emojiList.firstWhere(
        (element) => element.name == "4349",
      );
      stickerPackageList.add(
        CustomStickerPackage(
          ...
          stickerList: qqEmojiSet.list
              ...
              .keys
              .map(
                (idx) => CustomSticker(index: idx, name: qqEmojiSet.list[idx]),
              )
              .toList(),
          menuItem: CustomSticker(index: 0, name: qqEmojiSet.icon),
        ),
      );
    }
    if (stickerConfig.unicodeEmojiList.isNotEmpty) {
      final defEmojiList = TUIKitStickerConstData.defaultUnicodeEmojiList.map((
        emojiItem,
      ) {
        return CustomSticker(
          index: 0,
          name: emojiItem.toString(),
          unicode: emojiItem,
        );
      }).toList();
      stickerPackageList.add(
  ...
  void setCurrentCursor(int? value) {
    currentCursor = value;
  }
  ...
  void deleteStickerFromText() {
    ...
    if (originalText == zeroWidthSpace) {
      ...
      if (cursorPosition > 0) {
        ...
        int removeLength = 1;
        int openBracketIndex = originalText.lastIndexOf(
          '[',
          cursorPosition - 1,
        );
        ...
        text =
            originalText.substring(0, cursorPosition - removeLength) +
  ...
  void onCustomEmojiFaceSubmitted(int index, String data) {
    final convType = widget.conversationType;
  ...
  void _handleSoftKeyBoardDelete() {
    if (widget.model.repliedMessage != null) {
  ...
  void mentionMemberInMessage(String? userID, String? nickName) {
    if (TencentUtils.checkString(userID) == null) {
      focusNode.requestFocus();
    } else {
      final memberInfo =
          widget.model.groupMemberList?.firstWhereOrNull(
            (element) => element?.userID == userID,
          ) ??
          V2TimGroupMemberFullInfo(userID: userID ?? "", nickName: nickName);
  ...
  Offset getAtPosition(String text, int atPlace) {
    ...
    final TextPosition lastLineOffset = textPainter.getPositionForOffset(
      Offset(textPainter.width, textPainter.height),
    );
    final Offset caretPosition = textPainter.getOffsetForCaret(
      lastLineOffset,
      Rect.zero,
    );
  ...
  void calculateRemoveRemainAt(String text) {
  ...
  Future<void> _handleAtText(
    ...
    if (isDesktopScreen) {
      (int, String, bool)? changedCharacterRecord = findChangedCharacter(
        originalText,
        text,
      );
      ...
      if (changedTextPosition != null) {
        ...
        if (atPlace != -1) {
          keyword = text.substring(
            atPlace + 1,
            changedTextPosition + (isAdded ? 1 : 0),
          );
        }
      } else {
        atPlace = -1;
      ...
      if (atPlace >= 0) {
        ...
        showAtMemberList.sort((
          V2TimGroupMemberFullInfo userA,
          V2TimGroupMemberFullInfo userB,
        ) {
  ...
  void replaceAtTag(String selectedMember) {
    ...
    int atIndex = textEditingController.text.lastIndexOf(
      '@',
      cursorPosition - 1,
    );
    if (atIndex >= 0) {
      ...
      textEditingController.selection = TextSelection.collapsed(
        offset: atIndex + selectedMember.length + 2,
      );
  ...
  KeyEventResult handleDesktopKeyEvent(FocusNode node, RawKeyEvent event) {
    ...
    if (event.runtimeType == RawKeyDownEvent) {
      ...
      if (event.isKeyPressed(LogicalKeyboardKey.arrowUp) &&
        ...
        widget.atMemberPanelScroll?.scrollToIndex(
          newIndex,
  ...
  void controllerHandler() {
    ...
    if (actionType == ActionType.longPressToAt) {
      mentionMemberInMessage(
        widget.controller?.atUserID,
        widget.controller?.atUserName,
      );
  ...
  @override
  void didUpdateWidget(TIMUIKitInputTextFieldPro oldWidget) {
  ...
  Future<void> _getMuteType(TUIChatSeparateViewModel model) async {
    if (!mounted) {
      return;
    }
  ...
  void _handleSendEditStatus(String value, bool status) {
        ...
        return LayoutBuilder(
          builder: (BuildContext context, BoxConstraints constraints) {
            ...
            return TUIKitScreenUtils.getDeviceWidget(
              context: context,
              defaultWidget: TIMUIKitTextFieldLayoutNarrowPro(
              ...
              desktopWidget: TIMUIKitTextFieldLayoutWide(
                ...
                addStickerToText: addStickerToText,
                customStickerPanel: widget.customStickerPanel,
                // forbiddenText: forbiddenText, //todo check
                onChanged: widget.onChanged,
                backgroundColor: widget.backgroundColor,
                morePanelConfig: widget.morePanelConfig,
                repliedMessage: value,
