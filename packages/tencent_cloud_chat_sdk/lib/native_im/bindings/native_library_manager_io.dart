...
import 'package:tencent_cloud_chat_sdk/native_im/tools_io.dart';
...
const String APIIsBase64EncodedKey = 'isBase64Encoded';
class NativeLibraryManager {
  ...
  static void _handleNativeCallback(String callbackName, Map<String, dynamic> dataFromNativeMap) async {
    switch (callbackName) {
      case "apiCallback":
        ...
        String jsonParamRaw = dataFromNativeMap[APIJsonParamKey] ?? '';
        bool isBase64Encoded = dataFromNativeMap[APIIsBase64EncodedKey] ?? false;
        
        String jsonParam = '';
        if (jsonParamRaw.isNotEmpty) {
          if (isBase64Encoded) {
            try {
              List<int> bytes = base64Decode(jsonParamRaw);
              // allowMalformed: true: 允许错误格式的UTF-8解码
              jsonParam = utf8.decode(bytes, allowMalformed: true);
            } catch (e) {
              jsonParam = '';
              print("Decode Exception e = $e");
            }
          } else {
            jsonParam = jsonParamRaw;
          }
        }

        dataFromNativeMap[APIJsonParamKey] = jsonParam;
  ...
  /// 从全局回调数据中获取指定 key 的字符串值，自动处理 base64 解码
  static String _getGlobalCallbackJsonString(Map<String, dynamic> dataFromNativeMap, String key) {
    ...
    if (isBase64Encoded) {
      try {
        List<int> bytes = base64Decode(rawValue);
        return utf8.decode(bytes, allowMalformed: true);
      } catch (e) {
        print("_getGlobalCallbackJsonString Decode Exception key=$key, e=$e");
        return '';
      }
    }
